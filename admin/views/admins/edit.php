...
<form id="product-entry-form" enctype="multipart/form-data" method="post" action="update.php" role="form">
    ...
    <input id="id"  value="<?php echo $admin['picture']?>" type="hidden" name="pre_picture" class="form-control">
    </div>
...
if(!empty($admin['picture'])){
    ?>
    <div><img class="col-3" src="<?=UPLOADS;?><?php echo $admin['picture']?>"  alt="<?php echo $admin['first_name'].$admin['last_name']?>"></div>
 <?php
} else{
    ?>
